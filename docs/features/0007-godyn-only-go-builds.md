...
promotion-criteria: |
  ...
  the committed graph cannot silently drift from the source (a check
  fails when it does).
...
an edit to one package, bobo (spinclass/fast-aspen) measured about 3.7 s for the
godyn build against about 34 s for buildGoApplication. The same per-package
...
- **Packages** are `buildGodynModule` (or `buildGoAuto` while a consumer
  migrates) over a committed `godyn-gen` graph.
...
| Platforms | x86_64-linux proven (flac, twerk) | darwin / aarch64 unvalidated (igloo#33) |
| Graph freshness | committed graph, regenerated by hand | no check fails on drift (not yet filed) |
| Workspace consumers | `-gomod` takes a merged go.mod | go.work consumers (not yet filed) |
...
- **Committed graphs.** A graph describes one platform and one dependency
  set; a stale graph builds the wrong file set. The workflow must make drift
  loud (a check comparing the committed graph with a fresh one) before godyn
...
- **Cold builds.** Per-package derivations cost more than one `go build` from
  ...
  accepted; binary caching (krone) is what keeps cold consumers fast.
