...
let
  ...
  mkProd =
    name: passthru: pkgs.runCommand name { passthru.goFlakeInputs = passthru; } "mkdir -p $out";
  ...
  cyclePair = rec {
    a = pkgs.runCommand "cycle-a" {
      passthru.goFlakeInputs = {
        "example.com/cb" = b;
      };
    } "mkdir -p $out";
    b = pkgs.runCommand "cycle-b" {
      passthru.goFlakeInputs = {
        "example.com/ca" = a;
      };
    } "mkdir -p $out";
