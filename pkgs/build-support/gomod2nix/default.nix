...
let
  ...
  # external dep expressed as `replace <mod> => <store path>`, then run
  # `go work vendor` so GO generates vendor/ + modules.txt — it owns the
  # MVS/version-skew reconciliation the hand-written mkWorkspaceModulesTxt
  # cannot. Producers become replaced (vendored) deps, not `use` members, so
  # a consumer's organic `require <producer> <version>` resolves cleanly
  # (a versioned require for a `use`d member would otherwise fail), and the
  # consumer's real require versions stand — no sentinel.
  #
  # NB: `go work vendor` reads the consumer source to compute the import
  # graph, so this derivation is source-dependent (weaker caching than
  # mkVendorEnv's gomod2nix.toml-keyed vendor) — acceptable for the
  # experimental workspace mode. The consumer's go.mod is NOT modified.
  # `passthru.goWorkFile` is the same go.work the build must materialize so
  # the `-mod=vendor` build runs against an identical workspace.
  mkGoWorkVendorEnv =
    {
      go,
      src,
    ...
    let
      ...
      goWorkContent = ''
        ...
        ${concatStringsSep "\n" (producerReplaces ++ externalReplaces)}
      '';

      goWorkFile = writeText "merged-go.work" goWorkContent;
    in
    runCommand "go-work-vendor-env"
      {
        nativeBuildInputs = [ go ];
        passthru = { inherit goWorkFile goWorkContent; };
      }
      ''
  ...
  # The registry toolchain `goToolchain.go` (the newest go-toolchain(7) entry,
  # what godyn builds with; FDR 0012) wins whenever it satisfies go.mod's
  # directive; the scan over nixpkgs' go_<x>_<y> attributes is the fallback for
  # a go.mod that demands a newer minor than the registry has. nixpkgs' plain
  # `go` is never selected implicitly.
  registryGo = buildPackages.goToolchain.go;
  selectGo =
    attrs: goMod:
    attrs.go or (
      if goMod == null then
        registryGo
      else if lib.versionAtLeast registryGo.version goMod.go then
        registryGo
      else
        (
          let
  ...
  mkGoEnv =
    ...
    let
      ...
      # generated by `go work vendor` (mkGoWorkVendorEnv), matching
      # buildGoApplication. null in replace mode → mkVendorEnv unchanged.
      bridgeVendorEnv =
        if workspaceBridge then
          mkGoWorkVendorEnv {
            inherit go normalizedFlakeInputs modulesStruct;
            src = effectivePwd;
            goVersion = consumerGoModForVersion.go;
          }
        else
          null;

      vendorEnv =
        if bridgeVendorEnv != null then
          bridgeVendorEnv
        else
          mkVendorEnv {
            inherit
  ...
  buildGoApplication =
    ...
    let
      ...
      # generated by `go work vendor` (mkGoWorkVendorEnv) so go owns the
      # MVS/version-skew reconciliation. `passthru.goWorkFile` is the exact
      # go.work the build materializes so `-mod=vendor` runs against the same
      # workspace. The go cache is disabled for bridges (the cache derivation
      # reconstructs an in-tree source/ that can't carry out-of-tree `use`/
      # `replace` targets) — a perf follow-up, not a correctness gap.
      bridgeVendorEnv =
        if workspaceBridge then
          mkGoWorkVendorEnv {
            inherit go normalizedFlakeInputs modulesStruct;
          ...
          null;
      effectiveDisableGoCache = disableGoCache || workspaceBridge;
    ...
    stdenv.mkDerivation (
      ...
      // {
        ...
        postPatch =
          ...
          ''
          # Workspace mode (Design A, igloo#39): materialize the SAME go.work
          # that mkGoWorkVendorEnv used (`use .` + replaces), so the rsynced
          # vendor/ + the hook's `-mod=vendor` build run against an identical
          # workspace. Placed before goConfigHook (in postPatchHooks) so it
          # sits alongside the consumer go.mod at the build root. The replace
          # target store paths are pulled into the closure via goWorkFile.
          + optionalString workspaceBridge ''
            cp --no-preserve=mode ${bridgeVendorEnv.passthru.goWorkFile} go.work
          ''
          + (attrs.postPatch or "");
  ...
  buildGoCheck =
    ...
    base.overrideAttrs (
      ...
      // {
        ...
        # The seed already holds the base's go-cache-env; skip the hook's
        # restore rather than unpack it twice.
        goCacheDir = if cacheSeed != null then "" else old.goCacheDir or "";
        ...
        # by now, so the tool sees the bridged, vendored module graph.
        buildPhase = ''
          runHook preBuild
          ${lintScratchEnv}
          ${optionalString (cacheSeed != null) ''
            echo "buildGoCheck: seeding GOCACHE and GOLANGCI_LINT_CACHE from ${cacheSeed}"
            ${unpackLintCache cacheSeed "GOCACHE" "gocache.tar.zst"}
            ${unpackLintCache cacheSeed "GOLANGCI_LINT_CACHE" "golangci-lint-cache.tar.zst"}
  ...
  # HOME and the golangci-lint cache must be writable in both lint lanes — the
  ...
  # golangci-lint keys dependency packages by absolute file path, so the lint
  # and its warm-up unpack to one fixed directory to see identical paths.
  moveToLintSourceDir = ''
    mv "$sourceRoot" go-lint-src
    sourceRoot=go-lint-src
  '';

  # The base's install hooks, install checks, and extra outputs all assume its
  # binary; the lint lanes produce only a marker or cache tarballs in $out.
  noInstallHooks = {
    preInstall = "";
    postInstall = "";
  ...
  packLintCache = var: file: ''
    ${gnutar}/bin/tar -cf - -C "${"$" + var}" . | ${zstd}/bin/zstd -T$NIX_BUILD_CORES -o "$out/${file}"
  '';

  unpackLintCache = seed: var: file: ''
    mkdir -p "${"$" + var}"
    ${zstd}/bin/zstd -d -c ${seed}/${file} | ${gnutar}/bin/tar -xf - -C "${"$" + var}"
  '';
    ...
    base.overrideAttrs (
      ...
      // {
        ...
        src = lib.cleanSourceWith {
          inherit (base) src;
          name = "go-lint-deps";
          filter = path: type: type != "directory" && builtins.elem (baseNameOf path) lintDepFileNames;
        };
        postUnpack = moveToLintSourceDir;
        ...
        '';
        doCheck = false;
        # The base's build hooks target first-party source, absent here.
        preBuild = "";
        postBuild = "";
        buildPhase = ''
          runHook preBuild
          ...
          # A library package, not cachegen's `package main`: a main package
          # pulls runtime build output into the seed (150 KiB -> 17 MiB for a
          ...
          mkdir zz_lint_cache_warm
          {
            ...
            echo 'import ('
            if [ -n "$vendored" ]; then
              go list -e -f '{{if and (ne .Name "main") (not .Incomplete)}}_ "{{.ImportPath}}"{{end}}' $vendored
            fi
            echo ')'
          ...
          runHook postBuild
        '';
        # GOCACHE entries embed the Go toolchain's store path, and whether the
        # reference scanner spots it inside the zstd stream is down to how the
        # bytes compress. Record it explicitly instead, so the seed always
        # references the go it is valid for.
        disallowedReferences = [ ];
        installPhase = ''
          runHook preInstall
          mkdir -p "$out"
          echo ${base.passthru.go} > "$out/go"
          ${packLintCache "GOCACHE" "gocache.tar.zst"}
          ${packLintCache "GOLANGCI_LINT_CACHE" "golangci-lint-cache.tar.zst"}
          runHook postInstall
        '';
        dontFixup = true;
      }
  ...
  buildGoLint =
    {
      ...
      # while the warm cache is experimental. The seed's dependency facts are
      # keyed by absolute path, so they only hit where the build directory is
      # identical across builds (/build in the Linux sandbox; a Linux build
      # without the sandbox misses too). Off Linux the seed is dropped.
      warmCache ? false,
      extraArgs ? [ ],
    }:
    let
