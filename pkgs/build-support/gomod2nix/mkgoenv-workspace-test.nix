...
#
# mkGoEnv must accept goFlakeInputsMode = "workspace", build a devshell env
# whose vendor/ is generated by `go work vendor` (mkGoWorkVendorEnv), and
...
let
  ...
  # Consumer organically requires the producer (the real-consumer case); the
  # go.work `replace` redirects it to the producer source. The tools.go is
  # what makes mkGoEnv materialize the workspace + run go work vendor.
...
pkgs.runCommand "mkgoenv-workspace-test"
  ...
  ''
    ...
    cat "$goWork"
    grep -q 'use \.' "$goWork" || { echo "FAIL: no 'use .' member"; exit 1; }
    grep -Eq 'replace example.com/prod/v2 => /nix/store/.*mkgoenv-test-prod-v2' "$goWork" \
      || { echo "FAIL: producer not replaced in go.work"; exit 1; }
    # Sentinel-free: the overlay carries no synthetic version.
